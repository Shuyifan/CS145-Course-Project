...
def train_logistic(learning_rate=0.01, training_epochs=50, batch_size=100):
    ...
    def softmax(z, axis=-1):
        zp = tf.reduce_max(z, axis=axis, keepdims=True)
        zexp = tf.exp(z - zp)
        return zexp / tf.reduce_sum(zexp, axis=axis, keepdims=True)

    pred = softmax(tf.matmul(x, W) + b)
    loss = tf.reduce_mean(-tf.reduce_sum(y * tf.log(pred + 1e-9), axis=1))

    optimizer = tf.train.GradientDescentOptimizer(learning_rate).minimize(loss)
    return optimize(x, y, pred, loss, optimizer, training_epochs, batch_size)


def train_nn(learning_rate=0.2, training_epochs=50, batch_size=50, n_hidden=64):
    x = tf.placeholder(tf.float32, [None, 784])
    y = tf.placeholder(tf.float32, [None, 10])
    ...
    xp = tf.tanh(tf.matmul(x, W1) + b1)
    pred = softmax(tf.matmul(xp, W2) + b2)
    loss = tf.reduce_mean(-tf.reduce_sum(y * tf.log(pred + 1e-9), axis=1))
