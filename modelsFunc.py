...
"""
The function to perform a random forest classifier based on a training data. Then calculate the training error and testing 
    error.
...
:rtype test_error: float
"""
def runRandomForestClassifier(train_X, train_y, validate_X, validate_y, max_depth = None):
...
"""
The function to perform a gradient boosting regression based on a training data. Then calculate the training error and testing 
    error.
...
:rtype test_error: float
"""
def runGradientBoostingRegressor(train_X, train_y, validate_X, validate_y, max_depth = None):
    clf = GradientBoostingRegressor(n_estimators=100, max_depth = max_depth, loss = 'ls')
    clf.fit(train_X, train_y)
    train_error = mse(train_y, clf.predict(train_X))**0.5
    test_error = mse(validate_y, clf.predict(validate_X)) ** 0.5
    print (train_error)
